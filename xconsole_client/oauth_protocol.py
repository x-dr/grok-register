    ...
    def __init__(
        ...
        *,
        ...
        if self._yescaptcha_key:
            # Support local turnstile-solver (YesCaptcha-compatible) via endpoint env.
            endpoint = (
            ...
            # Only force endpoint for explicit local key / local host.
            use_endpoint = None
            if endpoint and (
                self._yescaptcha_key == "local"
                or "127.0.0.1" in endpoint
                or "localhost" in endpoint
                or endpoint.rstrip("/").endswith(":5072")
            ):
                use_endpoint = endpoint.rstrip("/")
            self.solver = YesCaptchaSolver(
                self._yescaptcha_key,
                endpoint=use_endpoint,
                debug=debug,
                auto_fallback_endpoint=use_endpoint is None,
            )
        try:
